 ...
 * @author Pichau
 */
public class Menu implements CarParkManager {
    ...
    private CarPrototype car = new CarPrototype();
    private BikePrototype bike = new BikePrototype();
    private VanPrototype van = new VanPrototype();
    private ParkingSlotsMemento parkingMemento = new ParkingSlotsMemento();
//    Menu carParkObject = new Menu();
    //private static ParkingSlotsCareTaker parkingCareTaker = new ParkingSlotsCareTaker();

    private List<Integer> vehicleOrderList = new ArrayList<Integer>();//is used to have the index of the vehicles 
    ...
    public void displayMainMenu() {
        ...
        while (true) {//infinity loop
            ...
            System.out.println("2. Withdraw a vehicle from the parking Space");
    ...
    public void addVehicle() {
        ...
        switch (vehicleType) {
            case "car":
                ...
                CarPrototype carro = (CarPrototype) car.clonar();
                carro.setBrandName(brand);
                carro.setEntryTime(entryTime);
                carro.setPlateID(id);
                carro.setNoOfDoors(noOfDoors);
                carro.setCarColor(color);
                carro.setStrategy(new CarStrategy());

                vehicleParkingSlots[checkFreeSpace] = carro;
                break;
            ...
            case "van":
                ...
                VanPrototype van2 = (VanPrototype) van.clonar();
                van.setPlateID(id);
                van.setCargoVolume(volume);
                van.setEntryTime(entryTime);
                van.setBrandName(brand);
                van.setStrategy(new VanStrategy());
                vehicleParkingSlots[checkFreeSpace] = van;
                vehicleParkingSlots[checkFreeSpace + 1] = van;
            ...
            case "bike":
                ...
                BikePrototype motorBike = (BikePrototype) bike.clonar();
                motorBike.setPlateID(id);
                motorBike.setEntryTime(entryTime);
                motorBike.setBrandName(brand);
                motorBike.setEngineSize(size);
                motorBike.setStrategy(new BikeStrategy());
                vehicleParkingSlots[checkFreeSpace] = motorBike;
    ...
    public void deleteVehicle() {
        ...
        System.out.println("The value for the vehicle was : " + vehicleParkingSlots[i].calcValue(hours));

        if (vehicleParkingSlots[i] instanceof VanPrototype) {
    ...
    public void displayCurrentList() {
        ...
        for (int i = (vehicleOrderList.size() - 1); i >= 0; i--) {
            ...
            if(vehicleParkingSlots[index]!=null){
    ...
    public void CareTaker() {
       parkingMemento.setVehicleArray(Arrays.copyOf(vehicleParkingSlots,20), vehicleOrderList);
    }

    @Override
    public void Undo() {
        vehicleParkingSlots = parkingMemento.getUltimoSalvo();
        vehicleOrderList = parkingMemento.getUltimoInteger();
